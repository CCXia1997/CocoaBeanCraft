...
public class WorldGenTwillTrees extends WorldGenAbstractTree {
	...
	List<WorldGenTwillTrees.FoliageCoordinates> foliageCoords;

	public WorldGenTwillTrees(boolean p_i2027_1_) {
		super(p_i2027_1_);
	}
	...
	void generateLeafNodeList() {
		...
		this.foliageCoords = Lists.<WorldGenTwillTrees.FoliageCoordinates>newArrayList();
		this.foliageCoords.add(new WorldGenTwillTrees.FoliageCoordinates(this.basePos.up(k), j));
				...
				for (int l = 0; l < i; ++l) {
					...
					if (this.checkBlockLine(blockpos, blockpos1) == -1) {
						...
						if (this.checkBlockLine(blockpos2, blockpos) == -1) {
							this.foliageCoords
									.add(new WorldGenTwillTrees.FoliageCoordinates(blockpos, blockpos2.getY()));
						}
					}
				}
			}
	...
	void generateLeafNode(BlockPos pos) {
		for (int i = 0; i < this.leafDistanceLimit; ++i) {
			this.crosSection(pos.up(i), this.leafSize(i), ModBlocks.TWILL_CHOCOLATE_LEAVES.getDefaultState()
					.withProperty(BlockLeaves.CHECK_DECAY, Boolean.valueOf(false)));
		}
	}
	...
	void limb(BlockPos p_175937_1_, BlockPos p_175937_2_, Block p_175937_3_) {
		...
		for (int j = 0; j <= i; ++j) {
			...
			this.setBlockAndNotifyAdequately(this.world, blockpos1,
					p_175937_3_.getDefaultState().withProperty(BlockHollowChocolateLog.LOG_AXIS, blocklog$enumaxis));
		}
	}
	 ...
	 */
	void generateLeaves() {
		for (WorldGenTwillTrees.FoliageCoordinates worldgenbigtree$foliagecoordinates : this.foliageCoords) {
	...
	void generateTrunk() {
		...
		Block block = ModBlocks.TWILL_CHOCOLATE_LOG;
	...
	void generateLeafNodeBases() {
		for (WorldGenTwillTrees.FoliageCoordinates worldgenbigtree$foliagecoordinates : this.foliageCoords) {
			...
			if (!blockpos.equals(worldgenbigtree$foliagecoordinates)
				...
				this.limb(blockpos, worldgenbigtree$foliagecoordinates, ModBlocks.TWILL_CHOCOLATE_LOG);
			}
		}
	}

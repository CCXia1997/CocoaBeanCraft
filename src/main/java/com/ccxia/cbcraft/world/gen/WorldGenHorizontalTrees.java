...
public class WorldGenHorizontalTrees extends WorldGenAbstractTree {

	// 配置横纹树木的方块朝向
	private static final IBlockState TRUNK = ModBlocks.HORIZONTAL_CHOCOLATE_LOG.getDefaultState()
			.withProperty(BlockHollowChocolateLog.LOG_AXIS, BlockLog.EnumAxis.Y);
	private static final IBlockState LEAF = ModBlocks.HORIZONTAL_CHOCOLATE_LEAVES.getDefaultState()
			.withProperty(BlockLeaves.CHECK_DECAY, Boolean.valueOf(false));

	public WorldGenHorizontalTrees(boolean p_i2027_1_) {
		super(p_i2027_1_);
	}
	...
	public boolean generate(World worldIn, Random rand, BlockPos position) {
		...
		if (position.getY() >= 1 && position.getY() + i + 1 <= 256) {
			...
			if (!flag) {
				...
				boolean isSoil = state.getBlock() == ModBlocks.OPERA_BLOCK;

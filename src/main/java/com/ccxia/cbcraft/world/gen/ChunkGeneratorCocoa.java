...
import com.ccxia.cbcraft.world.biome.BiomeBrownieHill;
import com.ccxia.cbcraft.world.biome.BiomeOpera;
import com.ccxia.cbcraft.world.biome.BiomeSchwarezwaelder;
import com.ccxia.cbcraft.world.biome.BiomeTiramisuPlain;
...
public class ChunkGeneratorCocoa implements IChunkGenerator {
	// 可可岩代替石头
	protected static final IBlockState STONE = ModBlocks.COCOA_STONE.getDefaultState();
	...
	public ChunkGeneratorCocoa(World worldIn, long seed, boolean mapFeaturesEnabledIn, String generatorOptions) {
		{
...
//					net.minecraftforge.event.terraingen.InitMapGenEvent.EventType.WOODLAND_MANSION);
		...
		// WorldType
		this.terrainType = WorldType.parseWorldType("cocoa_world");
	...
	public void replaceBiomeBlocks(int x, int z, ChunkPrimer primer, Biome[] biomesIn) {
		...
		for (int i = 0; i < 16; ++i) {
			for (int j = 0; j < 16; ++j) {
				...
				if (biome instanceof BiomeTiramisuPlain) {
					((BiomeTiramisuPlain) biome).generateBiomeTiramisuPlainTerrain(this.world, rand, primer, x * 16 + i,
							z * 16 + j, this.depthBuffer[j + i * 16]);
				} else if (biome instanceof BiomeBrownieHill) {
					((BiomeBrownieHill) biome).generateBiomeBrownieHillTerrain(this.world, rand, primer, x * 16 + i,
							z * 16 + j, this.depthBuffer[j + i * 16]);
				} else if (biome instanceof BiomeSchwarezwaelder) {
					((BiomeSchwarezwaelder) biome).generateBiomeBrownieHillTerrain(this.world, rand, primer, x * 16 + i,
							z * 16 + j, this.depthBuffer[j + i * 16]);
				} else if (biome instanceof BiomeOpera) {
					((BiomeOpera) biome).generateBiomeBrownieHillTerrain(this.world, rand, primer, x * 16 + i,
							z * 16 + j, this.depthBuffer[j + i * 16]);
				} else {

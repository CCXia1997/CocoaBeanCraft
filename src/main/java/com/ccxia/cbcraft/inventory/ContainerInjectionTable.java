...
public class ContainerInjectionTable extends Container {
	...
	private TileEntityInjectionTable tileEntity;
	...
	public ContainerInjectionTable(InventoryPlayer playerInventory, World world, BlockPos blockPos, EntityPlayer player,
		...
		this.addSlotToContainer(new Slot(this.outputSlot, 0, 111, 26) {
			...
			public ItemStack onTake(EntityPlayer thePlayer, ItemStack stack) {
				...
				ContainerInjectionTable.this.world.playSound((EntityPlayer) null, ContainerInjectionTable.this.blockPos,
						SoundEvents.BLOCK_ENCHANTMENT_TABLE_USE, SoundCategory.BLOCKS, 1.0F,
						ContainerInjectionTable.this.world.rand.nextFloat() * 0.1F + 1.7F);
				return stack;
			}
		});
		...
		this.tileEntity = (TileEntityInjectionTable) tileEntity;
	}
	...
	public boolean canInteractWith(EntityPlayer playerIn) {
		if (this.world.getBlockState(this.blockPos).getBlock() != INJECTION_TABLE) {
			...
			return playerIn.getDistanceSq((double) this.blockPos.getX() + 0.5D, (double) this.blockPos.getY() + 0.5D,
	...
	private void updateInjectionOutput() {
		...
		if (inputStack.isEmpty())
			return;
		// 这边根据TileEntity传回来的等级选择对应的配方
		Optional<ItemStack> outputStack;
		// 根据等级选择应该调用的常量配方对象
		if (tileEntity.getLevel() == 0) {
			outputStack = RecipesInjection.INJECTION_MANAGER.getOutput(inputStack);
		} else {
			outputStack = RecipesInjection.HIGH_INJECTION_MANAGER.getOutput(inputStack);
		}
		if (!outputStack.isPresent())
			return;
		if (tileEntity.getLevel() == 0) {
			this.cost = RecipesInjection.INJECTION_MANAGER.getCost(inputStack);
		} else {
			this.cost = RecipesInjection.HIGH_INJECTION_MANAGER.getCost(inputStack);
		}

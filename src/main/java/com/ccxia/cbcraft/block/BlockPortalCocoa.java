...
public class BlockPortalCocoa extends BlockBreakable {
	...
	public boolean trySpawnPortal(World worldIn, BlockPos pos) {
		BlockPortalCocoa.CocoaSize blockportal$size = new BlockPortalCocoa.CocoaSize(worldIn, pos, EnumFacing.Axis.X);

		if (blockportal$size.isValid() && blockportal$size.portalBlockCount == 0) {
			blockportal$size.placePortalBlocks();
			return true;
		} else {
			BlockPortalCocoa.CocoaSize blockportal$size1 = new BlockPortalCocoa.CocoaSize(worldIn, pos,
					EnumFacing.Axis.Z);
	...
	public void neighborChanged(IBlockState state, World worldIn, BlockPos pos, Block blockIn, BlockPos fromPos) {
		...
		if (enumfacing$axis == EnumFacing.Axis.X) {
			BlockPortalCocoa.CocoaSize blockportal$size = new BlockPortalCocoa.CocoaSize(worldIn, pos,
					EnumFacing.Axis.X);
	...
	public BlockPattern.PatternHelper createPatternHelper(World worldIn, BlockPos p_181089_2_) {
		...
		BlockPortalCocoa.CocoaSize blockportal$size = new BlockPortalCocoa.CocoaSize(worldIn, p_181089_2_,
				EnumFacing.Axis.X);
		LoadingCache<BlockPos, BlockWorldState> loadingcache = BlockPattern.createLoadingCache(worldIn, true);

		if (!blockportal$size.isValid()) {
			enumfacing$axis = EnumFacing.Axis.X;
			blockportal$size = new BlockPortalCocoa.CocoaSize(worldIn, p_181089_2_, EnumFacing.Axis.Z);
		}
	...
	public static class CocoaSize {
		...
		public CocoaSize(World worldIn, BlockPos p_i45694_2_, EnumFacing.Axis p_i45694_3_) {
			this.world = worldIn;
			...
			System.out.println("" + this.height + "    " + this.width + "    " + this.portalBlockCount);
		}
		...
		protected int getDistanceUntilEdge(BlockPos p_180120_1_, EnumFacing p_180120_2_) {
			...
			for (i = 0; i < 22; ++i) {
				...
				if (!this.isEmptyBlock(this.world.getBlockState(blockpos).getBlock())
						|| (block != ModBlocks.INJECTED_DARK_CHOCOLATE && block != ModBlocks.INJECTED_MILK_CHOCOLATE
								&& block != ModBlocks.INJECTED_WHITE_CHOCOLATE)) {
					break;
				}
			}

			Block block = this.world.getBlockState(p_180120_1_.offset(p_180120_2_, i)).getBlock();
			return (block == ModBlocks.INJECTED_DARK_CHOCOLATE || block == ModBlocks.INJECTED_MILK_CHOCOLATE
					|| block == ModBlocks.INJECTED_WHITE_CHOCOLATE) ? i : 0;
		}
		...
		protected int calculatePortalHeight() {
			...
			for (this.height = 0; this.height < 21; ++this.height) {
				for (int i = 0; i < this.width; ++i) {
					...
					if (block == ModBlocks.COCOA_PORTAL_BLOCK) {
					...
					if (i == 0) {
						...
						if (block != ModBlocks.INJECTED_DARK_CHOCOLATE && block != ModBlocks.INJECTED_MILK_CHOCOLATE
								&& block != ModBlocks.INJECTED_WHITE_CHOCOLATE) {
							break label56;
						}
			...
			for (int j = 0; j < this.width; ++j) {
				Block block = this.world.getBlockState(this.bottomLeft.offset(this.rightDir, j).up(this.height))
						.getBlock();
				if (block != ModBlocks.INJECTED_DARK_CHOCOLATE && block != ModBlocks.INJECTED_MILK_CHOCOLATE
						&& block != ModBlocks.INJECTED_WHITE_CHOCOLATE) {
					this.height = 0;
		...
		protected boolean isEmptyBlock(Block blockIn) {
			return blockIn.getDefaultState().getMaterial() == Material.AIR || blockIn == Blocks.FIRE
					|| blockIn == ModBlocks.COCOA_PORTAL_BLOCK || blockIn == ModBlocks.CHOCOLATE_CAKE;
		}
			...
			for (int i = 0; i < this.width; ++i) {
				...
				for (int j = 0; j < this.height; ++j) {
					this.world.setBlockState(blockpos.up(j),
							ModBlocks.COCOA_PORTAL_BLOCK.getDefaultState().withProperty(BlockPortal.AXIS, this.axis),
							2);
				}
			}
		}

...
public class BlockOpera extends Block {
	public BlockOpera() {
		super(Material.GRASS);
		this.setUnlocalizedName(CbCraft.MODID + ".operaBlock");
		this.setRegistryName("opera_block");
	...
	public boolean canSustainPlant(IBlockState state, IBlockAccess world, BlockPos pos, EnumFacing direction,
		...
		if (plant.getBlock() == ModBlocks.HORIZONTAL_CHOCOLATE_SAPLING) {
			return true;
		} else {
			return false;
		}
				...
				if (worldIn.getLightFromNeighbors(pos.up()) >= 9) {
					for (int i = 0; i < 4; ++i) {
						...
						if (iblockstate1.getBlock() == ModBlocks.COCOA_CAKE_BASE
							...
							worldIn.setBlockState(blockpos, ModBlocks.OPERA_BLOCK.getDefaultState());
						}
					}
				}
			}
